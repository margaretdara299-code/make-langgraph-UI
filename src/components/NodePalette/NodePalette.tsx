...
export default function NodePalette({ className = "" }: NodePaletteProps) {
  ...
  const totalActions = Object.values(filteredCategories).reduce(
  ...
  return (
    <aside className={`node-palette ${className}`}>
      <div className="node-palette__header">
        <Text strong className="node-palette__title">
          Node Library
        </Text>
      </div>
              ...
              items={[
                {
                  ...
                  label: (
                    <span className="node-palette__section-label">
                      <AppstoreFilled
                        style={{ color: "var(--node-palette-common-color)" }}
                      />{" "}
                      Common
                      <span className="node-palette__category-count">4</span>
                    </span>
                  ),
                ...
                },
                {
                  ...
                  label: (
                    <span className="node-palette__section-label">
                      <ThunderboltFilled
                        style={{ color: "var(--node-palette-actions-color)" }}
                      />{" "}
                      Actions
                    ...
                    categoryKeys.length === 0 ? (
                      ...
                      <Collapse
                        ...
                        items={categoryKeys.map((category) => ({
                          ...
                          children: filteredCategories[category].map(
                            (action) => (
                              <NodePaletteItem
                                key={action.id}
                                action={action}
